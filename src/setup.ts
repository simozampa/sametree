...
function createSetupDirectories(repositoryRoot: string, created: string[]): void {
  for (const relativePath of SETUP_DIRECTORIES) {
    const target = assertSafeWritePath(repositoryRoot, relativePath);
    try {
...
export function setupProject(
  ...
  const touched = [
    ...
    ...(openCodePlan
      ? [openCodePlan.config.relativePath, openCodePlan.instructions.relativePath]
      : []),
  ];
  ...
  try {
    createSetupDirectories(repository.root, createdDirectories);
    ...
    if (claudePlan?.addMcp) addClaudeServer(repository.root, runner);
